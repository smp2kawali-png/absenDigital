...
var API_URL = "URL_WEB_APP_GAS_ANDA_DISINI";
...
function callAPI(action, payload, callback) {
    ...
    .catch(err => { console.error(err); showToast('Koneksi Gagal', 'error'); });
}
...
function saveSettings() {
    callAPI('updateSettings', {settings:{jamTerlambat: document.getElementById('set-jam').value}}, function(r){ if(r.success) showToast('Tersimpan', 'success'); });
}
...
function startDirectFaceAttendance() {
    ...
    navigator.mediaDevices.getUserMedia({video: {facingMode:'user', width:640, height:480}}).then(stream => {
        ...
        document.getElementById('direct-video').srcObject = stream;
        ...
        Promise.all([
            ...
            document.getElementById('df-status').className = 'face-status success';
            document.getElementById('df-status').textContent = 'Kamera & AI Siap. Silakan Posisikan Wajah!';
            document.getElementById('df-btn').disabled = false;
        }).catch(() => {
            document.getElementById('df-status').className = 'face-status error';
            document.getElementById('df-status').textContent = 'Gagal memuat AI model';
        });
    }).catch(() => {
        document.getElementById('df-status').className = 'face-status error';
        document.getElementById('df-status').textContent = 'Kamera tidak diizinkan';
    });
}
...
function processDirectFace() {
    ...
    document.getElementById('df-status').className = 'face-status loading';
    document.getElementById('df-status').textContent = 'Mencocokkan wajah dengan database...';
    document.getElementById('df-btn').disabled = true;

    faceapi.detectSingleFace(cv, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks().withFaceDescriptor().then(d => {
        if(!d) {
            document.getElementById('df-status').className = 'face-status error';
            document.getElementById('df-status').textContent = 'Wajah tidak terdeteksi. Coba lagi!';
            document.getElementById('df-btn').disabled = false;
            return;
        }
        ...
        // Ambil data siswa dari server untuk dicocokkan secara lokal/efisien
        callAPI('getStudentList', {}, function(res) {
            ...
            var minDistance = 0.6; // Threshold kemiripan
            ...
            if(matchedStudent) {
                ...
                document.getElementById('df-status').className = 'face-status error';
                document.getElementById('df-status').textContent = 'Wajah tidak dikenali dalam sistem!';
                document.getElementById('df-btn').disabled = false;
            }
        });
    });
...
function submitSickLeave() {
    var bc = document.getElementById('s-bc').value;
    var st = document.getElementById('s-st').value;
    var kt = document.getElementById('s-kt').value;
    callAPI('getStudentByBarcode', {barcode: bc}, function(r){
        if(r.success) {
